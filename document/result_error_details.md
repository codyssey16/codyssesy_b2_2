...
본 시나리오에서는 핵심 모듈(OpenAI, Notion 등) 오류 시 이메일 알림 및 재시도(Retry)가 수행되도록 직렬 에러 핸들러 라인을 설계하였고, 이때 에러 핸들러 내부의 모듈(Send an Email, Google Sheets) 자체에서 발생할 수 있는 2차 오류에 대해서는 추가적인 핸들러를 넣지 않고 의도적으로 무시(Ignore)하도록 설계하였다.

이유: 에러 알림 모듈에 또다시 에러 핸들러를 붙일 경우, 워크플로우의 시각적 복잡도(지저분함)가 무한히 증가하는 '에러 핸들러 카스케이드(Cascade)' 현상이 발생하기 때문이다. 알림 서비스 자체의 장애 확률은 메인 서비스에 비해 극히 낮으므로, 이는 시스템의 가독성과 유지보수성을 위한 합리적이라고 판단했다. 다만, 이메일/구글 시트 모듈까지 동시에 실패하는 극단적인 예외 상황에 대비하여, Make의 시나리오 설정(Scenario settings)에서 **Store incomplete executions = Yes**로 지정하였다. 이로 인해 2차 에러로 시나리오가 중단되더라도 데이터가 유실되지 않고 '미완료 실행(Incomplete Execution)' 보관함에 저장되어 이메일 알림을 받고 추후 수동 복구가 가능하도록 2중 안전장치를 마련하였다.