...
| **비용 및 효율 최적화** | **[OpenAI (1차 ➡️ 2차)]** | 모든 기사를 요약하지 않고, 1차 LLM이 선별한 중요 기사 1건의 URL 본문만 2차 LLM이 요약하도록 파이프라인 구성 | **2단계 구조(Two-step Verification)를 통한 API 호출 제한** | 모든 피드 AI 기사에 무조건 요약 API를 호출하면 불필요한 토큰 비용이 발생함. 따라서 1차 검증(Title/Desc 기반 선별) 후 확정된 기사 1건당 최대 1회만 요약 프로세스를 실행하여 불필요한 비용 발생을 방지(Cost Optimization)함. |
